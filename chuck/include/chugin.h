...
// NOTE the ref assign accounts for the case that lhs == rhs and the reference==1: add BEFORE
// release
...
#define CK_DDN_SINGLE(f)                                                                 \
	f = (f >= 0 ? ((f > (t_CKSINGLE)1e-15 && f < (t_CKSINGLE)1e15) ? f : (t_CKSINGLE)0.0) \
	            : ((f < (t_CKSINGLE)-1e-15 && f > (t_CKSINGLE)-1e15) ? f : (t_CKSINGLE)0.0))
#define CK_DDN_DOUBLE(f)                                                     \
	f = (f >= 0 ? ((f > (t_CKDOUBLE)1e-15 && f < (t_CKDOUBLE)1e15) ? f : 0.0) \
...
extern "C" {
	...
	a_Stmt new_stmt_from_for(a_Stmt c1,
	                         a_Stmt c2,
	                         a_Exp c3,
	                         a_Stmt body,
	                         uint32_t line,
	                         uint32_t where);
	a_Stmt new_stmt_from_foreach(a_Exp iter,
	                             a_Exp array,
	                             a_Stmt body,
	                             uint32_t line,
	                             uint32_t where);
	a_Stmt new_stmt_from_loop(a_Exp cond, a_Stmt body, uint32_t line, uint32_t where);
	a_Stmt new_stmt_from_if(a_Exp cond,
	                        a_Stmt if_body,
	                        a_Stmt else_body,
	                        uint32_t line,
	                        uint32_t where);
	...
	a_Exp new_exp_from_unary2(ae_Operator oper,
	                          a_Type_Decl type,
	                          int ctor_invoked,
	                          a_Exp ctor_args,
	                          a_Array_Sub array,
	...
	a_Exp new_exp_from_cast(a_Type_Decl type,
	                        a_Exp exp,
	                        uint32_t line,
	                        uint32_t where,
	                        uint32_t castPos);
	...
	a_Exp new_exp_from_member_dot(a_Exp base,
	                              c_str member,
	                              uint32_t line,
	                              uint32_t where,
	                              uint32_t memberPos);
	...
	a_Exp new_exp_decl_external(a_Type_Decl type_decl,
	                            ...
	                            uint32_t line,
	                            uint32_t where);
	a_Exp new_exp_decl_global(a_Type_Decl type_decl,
	                          a_Var_Decl_List var_decl_list,
	                          int is_static,
	                          int is_const,
	                          uint32_t line,
	                          uint32_t where);
	a_Exp new_exp_decl(a_Type_Decl type_decl,
	                   a_Var_Decl_List var_decl_list,
	                   int is_static,
	                   int is_const,
	                   uint32_t line,
	...
	a_Var_Decl_List prepend_var_decl_list(a_Var_Decl var_decl,
	                                      a_Var_Decl_List list,
	                                      uint32_t line,
	                                      uint32_t where);
	a_Var_Decl new_var_decl(c_constr xid,
	                        int ctor_invoked,
	                        a_Exp ctor_args,
	                        a_Array_Sub array,
	                        uint32_t line,
	...
	a_Type_Decl add_type_decl_array(a_Type_Decl type_decl,
	                                a_Array_Sub array,
	                                uint32_t line,
	                                uint32_t where);
	a_Arg_List new_arg_list(a_Type_Decl type_decl,
	                        a_Var_Decl var_decl,
	                        uint32_t line,
	                        uint32_t where);
	a_Arg_List prepend_arg_list(a_Type_Decl type_decl,
	                            a_Var_Decl var_decl,
	                            a_Arg_List arg_list,
	                            uint32_t line,
	                            uint32_t where);
	...
	a_Class_Def new_class_def(ae_Keyword class_decl,
	                          a_Id_List xid,
	                          a_Class_Ext ext,
	                          a_Class_Body body,
	                          uint32_t line,
	...
	a_Class_Body prepend_class_body(a_Section section,
	                                a_Class_Body body,
	                                uint32_t line,
	                                uint32_t where);
	a_Class_Ext new_class_ext(a_Id_List extend_id,
	                          a_Id_List impl_list,
	                          uint32_t line,
	                          uint32_t where);
	a_Class_Def new_iface_def(ae_Keyword class_decl,
	                          a_Id_List xid,
	                          a_Class_Ext ext,
	                          a_Class_Body body,
	                          uint32_t line,
	...
	a_Id_List prepend_id_list(c_constr xid,
	                          a_Id_List list,
	                          uint32_t line,
	                          uint32_t where /*, YYLTYPE * loc*/);
	a_Func_Def new_func_def(ae_Keyword func_decl,
	                        ae_Keyword static_decl,
	                        a_Type_Decl type_decl,
	                        c_str name,
	                        a_Arg_List arg_list,
	                        a_Stmt code,
	                        uint32_t is_from_ast,
	                        uint32_t line,
	                        uint32_t where);
	a_Func_Def new_op_overload(ae_Keyword func_decl,
	                           ae_Keyword static_decl,
	                           a_Type_Decl type_decl,
	                           ae_Operator oper,
	                           a_Arg_List arg_list,
	...
	struct a_Exp_Dot_Member_ {
		...
		t_CKTYPE t_base;
		S_Symbol xid;
		t_CKBOOL isSpecialPrimitiveFunc;
		uint32_t line;
		uint32_t where;
		a_Exp self;
	};
	...
	struct a_Func_Def_ {
		...
		uint32_t
		   overload_post;  // 1.5.1.5 (ge) overload as post? e.g., @operator(args) ++ (unary only)
...
#define CK_DLL_DECLVERSION \
	CK_DLL_EXPORT(t_CKUINT) ck_version() { return CK_DLL_VERSION; }
...
#define CK_DLL_QUERY_STATIC(name) \
	CK_DLL_EXPORT(t_CKBOOL) CK_DLL_QUERY_STATIC_NAME(name)(Chuck_DL_Query * QUERY)
...
#define CK_DLL_ALLOC(name) \
	CK_DLL_EXPORT(Chuck_Object*) name(Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export constructors
// example: CK_DLL_CTOR(foo)
#define CK_DLL_CTOR(name) \
	CK_DLL_EXPORT(void)    \
	name(Chuck_Object* SELF, void* ARGS, Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export destructors
// example: CK_DLL_DTOR(foo)
#define CK_DLL_DTOR(name) \
	CK_DLL_EXPORT(void) name(Chuck_Object* SELF, Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export member functions
// example: CK_DLL_MFUN(foo)
#define CK_DLL_MFUN(name)                                                      \
	CK_DLL_EXPORT(void)                                                         \
	name(Chuck_Object* SELF, void* ARGS, Chuck_DL_Return* RETURN, Chuck_VM* VM, \
...
#define CK_DLL_SFUN(name)                                                    \
	...
	name(Chuck_Type* TYPE, void* ARGS, Chuck_DL_Return* RETURN, Chuck_VM* VM, \
	     Chuck_VM_Shred* SHRED, CK_DL_API API)
// example: CK_DLL_GFUN(foo) | 1.5.1.5 (ge & andrew) added for global-scope function, e.g., for op
// overloads
#define CK_DLL_GFUN(name) \
	CK_DLL_EXPORT(void)    \
	name(void* ARGS, Chuck_DL_Return* RETURN, Chuck_VM* VM, Chuck_VM_Shred* SHRED, CK_DL_API API)
// macro for defining ChucK DLL export ugen tick functions
// example: CK_DLL_TICK(foo)
#define CK_DLL_TICK(name) \
	CK_DLL_EXPORT(t_CKBOOL) name(Chuck_Object* SELF, SAMPLE in, SAMPLE* out, CK_DL_API API)
...
#define CK_DLL_TICKF(name) \
	...
	name(Chuck_Object* SELF, SAMPLE* in, SAMPLE* out, t_CKUINT nframes, CK_DL_API API)
...
#define CK_DLL_PMSG(name)                                                                     \
	CK_DLL_EXPORT(t_CKBOOL)                                                                    \
	name(Chuck_Object* SELF, const char* MSG, void* ARGS, Chuck_VM* VM, Chuck_VM_Shred* SHRED, \
	     CK_DL_API API)
// macro for defining ChucK DLL export uana tock functions
// example: CK_DLL_TOCK(foo)
#define CK_DLL_TOCK(name)  \
	CK_DLL_EXPORT(t_CKBOOL) \
	name(Chuck_Object* SELF, Chuck_UAna* UANA, Chuck_UAnaBlobProxy* BLOB, CK_DL_API API)
...
#define CK_DLL_SHREDS_WATCHER(name) \
	CK_DLL_EXPORT(void)              \
	name(Chuck_VM_Shred* SHRED, t_CKINT CODE, t_CKINT PARAM, Chuck_VM* VM, void* BINDLE)
...
#define CK_DLL_TYPE_ON_INSTANTIATE(name) \
	CK_DLL_EXPORT(void)                   \
	name(Chuck_Object* OBJECT, Chuck_Type* TYPE, Chuck_VM_Shred* SHRED, Chuck_VM* VM)
...
extern "C" {
	...
	typedef t_CKVOID(CK_DLL_CALL* f_ctor)(Chuck_Object* SELF,
	                                      void* ARGS,
	                                      Chuck_VM* VM,
	...
	typedef t_CKVOID(CK_DLL_CALL* f_dtor)(Chuck_Object* SELF,
	                                      ...
	                                      Chuck_VM_Shred* SHRED,
	                                      CK_DL_API API);
	typedef t_CKVOID(CK_DLL_CALL* f_mfun)(Chuck_Object* SELF,
	                                      void* ARGS,
	...
	typedef t_CKVOID(CK_DLL_CALL* f_sfun)(Chuck_Type* TYPE,
	                                      void* ARGS,
	                                      Chuck_DL_Return* RETURN,
	                                      Chuck_VM* VM,
	                                      Chuck_VM_Shred* SHRED,
	...
	typedef t_CKVOID(CK_DLL_CALL* f_gfun)(void* ARGS,
	                                      Chuck_DL_Return* RETURN,
	                                      Chuck_VM* VM,
	                                      Chuck_VM_Shred* SHRED,
	                                      CK_DL_API API);
	...
	typedef t_CKBOOL(CK_DLL_CALL* f_tickf)(Chuck_Object* SELF,
	                                       SAMPLE* in,
	                                       SAMPLE* out,
	                                       t_CKUINT nframes,
	                                       CK_DL_API API);
	typedef t_CKVOID(CK_DLL_CALL* f_ctrl)(Chuck_Object* SELF,
	                                      void* ARGS,
	                                      Chuck_DL_Return* RETURN,
	                                      Chuck_VM* VM,
	                                      Chuck_VM_Shred* SHRED,
	                                      CK_DL_API API);
	typedef t_CKVOID(CK_DLL_CALL* f_cget)(Chuck_Object* SELF,
	                                      void* ARGS,
	                                      Chuck_DL_Return* RETURN,
	                                      Chuck_VM* VM,
	                                      Chuck_VM_Shred* SHRED,
	                                      CK_DL_API API);
	typedef t_CKBOOL(CK_DLL_CALL* f_pmsg)(Chuck_Object* SELF,
	                                      const char* MSG,
	                                      void* ARGS,
	                                      Chuck_VM* VM,
	                                      Chuck_VM_Shred* SHRED,
	                                      CK_DL_API API);
	// uana specific
	typedef t_CKBOOL(CK_DLL_CALL* f_tock)(Chuck_Object* SELF,
	                                      Chuck_UAna* UANA,
	                                      Chuck_UAnaBlobProxy* BLOB,
	                                      CK_DL_API API);
	// "main thread" hook
	...
	typedef void(CK_DLL_CALL* f_shreds_watcher)(Chuck_VM_Shred* SHRED,
	                                            t_CKINT CODE,
	                                            t_CKINT PARAM,
	                                            Chuck_VM* VM,
	                                            void* BINDLE);
	// type instantiation callback
	typedef void(CK_DLL_CALL* f_callback_on_instantiate)(Chuck_Object* OBJECT,
	                                                     Chuck_Type* TYPE,
	                                                     Chuck_VM_Shred* originShred,
	                                                     Chuck_VM* VM);
}
...
extern "C" {
	...
	typedef void(CK_DLL_CALL* f_begin_class)(Chuck_DL_Query* query,
	                                         const char* name,
	                                         const char* parent);
	// add constructor, must be betwen begin/end_class : use f_add_arg to add arguments immediately
	// after
	...
	typedef void(CK_DLL_CALL* f_add_mfun)(Chuck_DL_Query* query,
	                                      f_mfun mfun,
	                                      const char* type,
	                                      const char* name);
	// add static function - args to follow
	typedef void(CK_DLL_CALL* f_add_sfun)(Chuck_DL_Query* query,
	                                      f_sfun sfun,
	                                      const char* type,
	                                      const char* name);
	// add binary operator overload - args included
	typedef void(CK_DLL_CALL* f_add_op_overload_binary)(Chuck_DL_Query* query,
	                                                    f_gfun gfun,
	                                                    const char* type,
	                                                    const char* op,
	                                                    const char* lhsType,
	...
	typedef void(CK_DLL_CALL* f_add_op_overload_prefix)(Chuck_DL_Query* query,
	                                                    f_gfun gfun,
	                                                    const char* type,
	                                                    const char* op,
	                                                    const char* argType,
	...
	typedef void(CK_DLL_CALL* f_add_op_overload_postfix)(Chuck_DL_Query* query,
	                                                     f_gfun gfun,
	                                                     const char* type,
	                                                     const char* op,
	                                                     const char* argType,
	...
	typedef void(CK_DLL_CALL* f_add_ugen_func)(Chuck_DL_Query* query,
	                                           f_tick tick,
	                                           f_pmsg pmsg,
	                                           t_CKUINT num_in,
	                                           t_CKUINT num_out);
	typedef void(CK_DLL_CALL* f_add_ugen_funcf)(Chuck_DL_Query* query,
	                                            ...
	                                            t_CKUINT num_in,
	                                            t_CKUINT num_out);
	typedef void(CK_DLL_CALL* f_add_ugen_funcf_auto_num_channels)(Chuck_DL_Query* query,
	                                                              f_tickf tickf,
	                                                              f_pmsg psmg);
	// ** add a ugen control (not used) | 1.4.1.0 removed
	// typedef void (CK_DLL_CALL * f_add_ugen_ctrl)( Chuck_DL_Query * query, f_ctrl ctrl, f_cget
	// cget,
	...
	// create main thread hook- used for executing a "hook" function in the main thread of a primary
	// chuck instance
	typedef Chuck_DL_MainThreadHook*(CK_DLL_CALL* f_create_main_thread_hook)(Chuck_DL_Query* query,
	                                                                         f_mainthreadhook hook,
	                                                                         f_mainthreadquit quit,
	                                                                         void* bindle);
	// register a callback to be called on host shutdown, e.g., for chugin cleanup
	typedef void(CK_DLL_CALL* f_register_callback_on_shutdown)(Chuck_DL_Query* query,
	                                                           f_callback_on_shutdown cb,
	                                                           void* bindle);
	// register a callback to be called on sample rate change
	typedef void(CK_DLL_CALL* f_register_callback_on_srate_update)(Chuck_DL_Query* query,
	                                                               f_callback_on_srate_update cb,
	                                                               void* bindle);
	// register a callback function to receive notification from the VM about shreds (add, remove,
	// etc.)
...
struct Chuck_DL_Func {
	...
	~Chuck_DL_Func();
	// add arg
	void add_arg(const char* t, const char* n) { args.push_back(new Chuck_DL_Value(t, n)); }
};
...
struct Chuck_DL_MainThreadHook {
public:
	Chuck_DL_MainThreadHook(f_mainthreadhook hook,
	                        f_mainthreadquit quit,
	                        void* bindle,
...
Chuck_DL_Return CK_DLL_CALL ck_invoke_mfun_immediate_mode(Chuck_Object* obj,
                                                          t_CKUINT func_vt_offset,
                                                          Chuck_VM* vm,
                                                          Chuck_VM_Shred* shred,
                                                          Chuck_DL_Arg* ARGS,
                                                          t_CKUINT numArgs);
...
public:
	...
	struct VMApi {
		...
		// queue an event; num_msg must be 1; buffer should be created using create_event_buffer()
		// above | 1.5.1.5
		t_CKBOOL(CK_DLL_CALL* const queue_event)(Chuck_VM* vm,
		                                         Chuck_Event* event,
		                                         t_CKINT num_msg,
		                                         CBufferSimple* buffer);
		// invoke Chuck_Object member function (defined either in chuck or c++) | 1.5.1.5 (ge &
		// andrew) NOTE this will call the member function in IMMEDIATE MODE, marking it as a
		// time-critical function when called in this manner; any time/event operations therein will
		// throw an exception
		Chuck_DL_Return(CK_DLL_CALL* const invoke_mfun_immediate_mode)(Chuck_Object* obj,
		                                                               t_CKUINT func_vt_offset,
		                                                               Chuck_VM* vm,
		                                                               Chuck_VM_Shred* shred,
		                                                               Chuck_DL_Arg* ARGS,
	...
	public:
		...
		// NOTE set addRef to TRUE if you intend to keep a reference of the newly created object
		// around (e.g., in the chugin) NOTE set addRef to FALSE if the created object is to be
		// returned without keeping a reference around
		...
		t_CKBOOL(CK_DLL_CALL* const get_mvar_vec2)(Object object,
		                                           const char* name,
		                                           t_CKVEC2& value);  // 1.5.2.0 | vec2/complex/polar
		t_CKBOOL(CK_DLL_CALL* const get_mvar_vec3)(Object object,
		                                           const char* name,
		                                           t_CKVEC3& value);  // 1.5.2.0 | vec3
		t_CKBOOL(CK_DLL_CALL* const get_mvar_vec4)(Object object,
		                                           const char* name,
		                                           t_CKVEC4& value);  // 1.5.2.0 | vec4
		...
		// get a chuck string's internal c-string (NOTE do not save a reference to the return value;
		// make a copy if needed) | 1.5.2.0
	...
	struct TypeApi {
		...
		// get vtable offset for named function (if overloaded, returns first one); returns < 0 if not
		// found
		...
		// register a callback to be invoked whenever a base-type (or its subclass) is instantiated,
		// with option for type system to auto-set shred origin if available
		void(CK_DLL_CALL* const callback_on_instantiate)(f_callback_on_instantiate callback,
		                                                 Type base_type,
		                                                 Chuck_VM* vm,
		...
		// get type name (full, with decorations) (NOTE do not save a reference to the return value;
		// make a copy if needed) | 1.5.2.0
		const char*(CK_DLL_CALL* const name)(Type type);
		// get type base name (no decorations) (NOTE do not save a reference to the return value; make
		// a copy if needed) | 1.5.2.0
	...
	// constructor
	Chuck_DL_Api() : vm(new VMApi), object(new ObjectApi), type(new TypeApi), shred(new ShredApi) {}

private:
	// make this object un-copy-able
	Chuck_DL_Api(Chuck_DL_Api& a) : vm(a.vm), object(a.object), type(a.type), shred(a.shred) {
		assert(0);
	};
...
public:
	...
	t_CKBOOL load(const char* filename, const char* func = CK_QUERY_FUNC, t_CKBOOL lazy = FALSE);
